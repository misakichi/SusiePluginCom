...
static HANDLE CreateFileMappingWWrapper(const wchar_t* name, DWORD size)
{
	SECURITY_ATTRIBUTES sa = {};
	sa.nLength = sizeof(sa);
	sa.bInheritHandle = FALSE;
	sa.lpSecurityDescriptor = nullptr;
	auto handle = CreateFileMappingW(INVALID_HANDLE_VALUE, &sa, PAGE_READWRITE, 0, size, name);
	return handle;
}
...
HRESULT STDMETHODCALLTYPE SharedMemory::CreateBuffer(DWORD size) 
{
	...
	do
	{
		if (FAILED(hr = CreateNamedObject(MappedFileGUID, s_FileID, std::bind(CreateFileMappingWWrapper, std::placeholders::_1, size)
			/*
			[size](const wchar_t* name) {
			SECURITY_ATTRIBUTES sa = {};
			sa.nLength = sizeof(sa);
			sa.bInheritHandle = FALSE;
			sa.lpSecurityDescriptor = nullptr;
		...
		if (FAILED(hr = CreateNamedObject(MutexGUID, s_MutexID, [](const wchar_t* name) {
				...
				sa.bInheritHandle = FALSE;
				sa.lpSecurityDescriptor = nullptr;
				auto handle = CreateMutexW(&sa, FALSE, name);
				return handle;
			}, &mutexObject)))
			break;
